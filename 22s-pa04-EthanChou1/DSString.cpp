...
DSString DSString::substring(int start, int numChars) {
    if(numChars <= strlen(word) && start >= 0)
    {
        char * temp = new char [(numChars - start) + 1];
        temp[(numChars - start) ] = '\0';
...
bool DSString::operator!=(const DSString & compareWord) {
    if (strcmp(this->word, compareWord.word) != 0) {
        return true;
    }
    else {
        return false;
